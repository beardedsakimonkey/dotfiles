...
// Gradient
//========================================================================
const float GRADIENT_STRENGTH = 0.2;
...
// Start/stop position of the gradient going from bottom to top of row (0→1).
const float GRADIENT_START = 0.0;
const float GRADIENT_STOP  = 1.0;

// Softens the gradient seam that can appear in double/squiggly underlines.
const float GRADIENT_SEAM_SOFTEN_PX = 5.0;
...
const float SHADOW_STRENGTH_RAMP_START = 0.2;
...
float backgroundSampleAgreementFromDistance(float distance)
{
    return 1.0 - smoothstep(0.01, 0.05, distance);
}

float backgroundSampleAgreement(vec3 a, vec3 b)
{
    return backgroundSampleAgreementFromDistance(colorDistance(a, b));
}

float contrastMask(vec3 color, vec3 background, float minContrast, float maxContrast)
{
    return smoothstep(minContrast, maxContrast, colorDistance(color, background));
}

float textMask(vec3 color, vec3 background)
{
    return contrastMask(color, background, 0.08, 0.35);
}
...
float shineLayerMask(vec2 uv, vec3 background, float text, float spreadPx)
{
    ...
    // Build a highlight band just inside the glyph edge: the near outward sample
    // must still be text, while the farther outward sample must be background.
    for (int i = 1; i <= 2; i++) {
        float insetPx = float(i);
        vec2 shineStep = vec2(0.0, insetPx) / iResolution.xy;
        vec2 outerStep = vec2(0.0, insetPx + max(spreadPx, 1.0)) / iResolution.xy;
        float innerText = sampleTextMask(uv - shineStep, background);
        float outerText = sampleTextMask(uv - outerStep, background);
        ...
        if (i == 1) {
            innerText *= 0.70;
            innerText += sampleTextMask(uv - shineStep - vec2(shineSpread.x, 0.0), background) * 0.15;
            innerText += sampleTextMask(uv - shineStep + vec2(shineSpread.x, 0.0), background) * 0.15;
        }

        float weight = 1.0 - ((insetPx - 1.0) / 2.0);
        shineMask += text * innerText * (1.0 - outerText) * weight;
        shineWeight += weight;
    }
...
)
{
    ...
    agreement = backgroundSampleAgreementFromDistance(distance);
...
{
    const float INSET_PX = 1.;
    const float MIN_AGREEMENT = 0.75;
    const float SPLIT_BLOCK_MIN_DELTA = 0.5;
    const float SIDE_SAMPLE_FRACTION = 0.12;
    const float SINGLE_EDGE_INNER_OFFSET_PX = 4.0;
    ...
    float rowTop = estimatedRowBottom(fragCoord.y, rowHeight);
    ...
    float rightSideX = estimatedColumnSampleX(
        ...
        cellWidth,
        0.0,
        1.0 - SIDE_SAMPLE_FRACTION
    );

    float leftSideAgreement = 0.0;
    vec3 leftSideColor = sampleRowEdgePair(leftSideX, rowTop, rowHeight, INSET_PX, leftSideAgreement);
    float rightSideAgreement = 0.0;
    vec3 rightSideColor = sampleRowEdgePair(rightSideX, rowTop, rowHeight, INSET_PX, rightSideAgreement);
    ...
    if (
        leftSideAgreement >= MIN_AGREEMENT &&
        rightSideAgreement >= MIN_AGREEMENT &&
        sideAgreement >= MIN_AGREEMENT
    ) {
        vec3 verticalTopColor = sampleSourceCoord(vec2(centerX, rowTop + 0.5));
        vec3 verticalCenterColor = sampleSourceCoord(vec2(centerX, rowTop + rowHeight * 0.5));
        vec3 verticalBottomColor = sampleSourceCoord(vec2(centerX, rowTop + rowHeight - 0.5));
        vec3 verticalColor = (verticalTopColor + verticalBottomColor) * 0.5;
        vec3 sideColor = (leftSideColor + rightSideColor) * 0.5;
        // Avoid gradient on vertical line characters like │ and ┃.
        if (
            backgroundSampleAgreement(verticalTopColor, verticalBottomColor) >= MIN_AGREEMENT &&
            backgroundSampleAgreement(verticalCenterColor, verticalColor) >= MIN_AGREEMENT &&
            colorDistance(sourceColor, verticalColor) <= 0.005 &&
            textMask(verticalColor, sideColor) > 0.0
        ) {
            return sourceColor;
        }
        return (
    ...
    if (
        leftSideAgreement >= MIN_AGREEMENT &&
        rightSideAgreement >= MIN_AGREEMENT &&
        colorDistance(leftSideColor, rightSideColor) >= SPLIT_BLOCK_MIN_DELTA
    ) {
        return sourceColor;
    }

    float innerInset = min(
        INSET_PX + SINGLE_EDGE_INNER_OFFSET_PX,
        max(rowHeight * 0.5 - 0.5, INSET_PX)
    );
    ...
    if (
        nearEdgeCleanAgreement >= MIN_AGREEMENT &&
        farEdgeCleanAgreement >= MIN_AGREEMENT
    ) {
        if (colorDistance(nearEdgeColor, farEdgeColor) >= SPLIT_BLOCK_MIN_DELTA) {
            return sourceColor;
        }
    ...
    if (
        nearEdgeCleanAgreement >= MIN_AGREEMENT &&
        farEdgeDistance >= SPLIT_BLOCK_MIN_DELTA
    ) {
        return sourceColor;
    }
    if (
        farEdgeCleanAgreement >= MIN_AGREEMENT &&
        nearEdgeDistance >= SPLIT_BLOCK_MIN_DELTA
    ) {
        return sourceColor;
    }
    if (
    ...
    if (nearEdgeCleanAgreement >= MIN_AGREEMENT) {
        return nearEdgeColor;
    }
    if (farEdgeCleanAgreement >= MIN_AGREEMENT) {
        return farEdgeColor;
    }
    if (
        colorDistance(nearEdgeColor, farEdgeColor) >= SPLIT_BLOCK_MIN_DELTA &&
        backgroundSampleAgreement(sourceColor, farEdgeColor) >= MIN_AGREEMENT
    ) {
        return farEdgeColor;
    }

    float centerAgreement = 0.0;
    vec3 centerColor = sampleRowEdgePair(centerX, rowTop, rowHeight, INSET_PX, centerAgreement);

    if (centerAgreement >= MIN_AGREEMENT) {
        return centerColor;
    }

    if (leftSideAgreement >= MIN_AGREEMENT) {
        return leftSideColor;
    }
    if (rightSideAgreement >= MIN_AGREEMENT) {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (GRADIENT_STRENGTH > 0.0) {
        float rowLocalY = fract((fragCoord.y + GRADIENT_Y_OFFSET_PX) / rowHeight) * rowHeight;
        float rowPosition = 1.0 - rowLocalY / rowHeight;
        float gradientPosition = smoothstep(
            GRADIENT_START,
            GRADIENT_STOP,
            rowPosition
        );
        float seamSoftener = smoothstep(0.0, GRADIENT_SEAM_SOFTEN_PX, rowLocalY);
        gradientPosition = min(gradientPosition, seamSoftener);
    ...
    if (SHINE_STRENGTH > 0.0) {
        ...
        // Scale the shine strength using the contrast between current pixel color and background color.
        float contrastScale = shineContrastFromBackground(source.rgb, background);
        float shineLift = shineMask * SHINE_STRENGTH * text * contrastScale;
